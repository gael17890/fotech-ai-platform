...
How 8 specialized agents, 4 tactical sub-agents, and an intent router work together to make a pharmacy have a conversation with itself.

## The decision: not one big agent

The first version was the obvious move — put everything behind a single Sonnet-4 super-agent with a giant system prompt and a long list of tools. Works for a demo. Falls over in production.

Three things killed the single-agent approach for me:

1. **The system prompt grew out of control.** Every new feature meant new rules in the prompt. By the time inventory, sales, restock, and onboarding all lived in one prompt, the model was tripping over rules from other domains.
2. **Cost scaled the wrong way.** Every request paid the price of the full prompt, even for a trivial intent.
3. **State machines don't compose in a single prompt.** Onboarding a product has clear states (`IDENTIFYING → ASKING_DATA → COLLECTING_LOTS → ... → DONE`). Sales doesn't. Mixing them into conversational rules made debugging a nightmare.

So I split the system into 8 specialized agents, each with a focused prompt, its own state machine when it needs one, and its own tools. An **intent router** picks which one handles each request.
...
This handles about 80% of intents. Free.
...
That separation is the whole point: **the router doesn't know pharmacy. The agents do.**
...
The two onboarding agents run explicit, validated state machines. Here's Solo Onboarding's:
...
Every transition is **validated** — the agent rejects messages that don't fit the current state. That's what keeps the conversation from going off the rails when the LLM gets confused.
...
When Bulk Onboarding runs into a new product and the investigator returns web search results, IDENTIFIER extracts a normalized record from raw HTML/text:
...
Why a separate Haiku call instead of inline in Sonnet? Cost and latency. Sonnet runs ~$3 in / $15 out per MTok. Haiku is $1 / $5. The IDENTIFIER call is ~600 input tokens, ~150 output. Sonnet would cost ~$0.004 a call. Haiku costs ~$0.001. Multiply that across thousands of products per onboarding session and it starts to matter.

### QUESTIONER

When data is missing, QUESTIONER writes the **minimum set of questions** to ask the admin. Not "fill all 20 fields" — only the fields the system genuinely can't infer.
...
The QUESTIONER prompt has rules like *"don't ask for concentration if it's in the brand name"*, *"don't ask for form if the presentation already encodes it"*.

### PHARMACIST

A virtual pharmacist with a clinical-reasoning prompt. Consulted when the system needs domain judgment that's hard to encode as rules:
...
The FILL stage of Bulk Onboarding hits PHARMACIST to populate fields the system can't determine deterministically.

### VALIDATOR

The last gate before the database write. VALIDATOR gets a complete proposed record and answers one question: *"is this internally consistent?"*
...
- Active ingredient and brand belong to different drugs

If VALIDATOR returns `false`, the record bounces back to QUESTIONER for clarification with the admin instead of committing a junk row.

## Why sub-agents, not just tool calls?

A tool call inside Sonnet is a function with a JSON signature. A sub-agent is its own LLM call with its own prompt.

I reach for a sub-agent when:
1. The call needs **its own system prompt** distinct from the parent agent's.
2. The work needs **reasoning** beyond a deterministic function.
3. **Cost or model selection** matters — Haiku here, Sonnet there.

Plain tool calls are for:
1. Deterministic work (database query, calculation).
2. Cases where **no prompt** is needed — just inputs and outputs.

Rule of thumb: **sub-agents for judgment, tool calls for facts.**
...
Agents don't import their tools directly. They receive them via constructor. Which means I can swap real tools for stubs in tests without spending money.
...
- **Only Bulk Onboarding is fully production-mature.** The others have stable routing and prompts but they're still being calibrated against real usage.
- **The L2 fallback isn't bulletproof.** It misclassifies about 5% of edge cases (regional Spanish slang, pharmacy jargon I haven't covered). The agent that gets a misrouted message politely hands it back.
- **State machines need maintenance.** Every new product field means revisiting the BRAIN_COMPLETING state. Tests catch the regressions.
