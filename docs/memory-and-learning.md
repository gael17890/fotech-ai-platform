...
Two distinct systems doing different work:

1. **Conversational memory** — what was said, by whom, when, indexed for full-text retrieval.
2. **Alias learning** — a continuously growing map from supplier-specific names to catalog products, built up from admin confirmations.

Together they make the platform sharper every day without any of the cost, complexity, or operational risk of fine-tuning.
...
CREATE TABLE aff_conversations (
  ...
  -- the trick: a generated tsvector column for Spanish full-text search
...
Every turn in every conversation gets a row. The `content_tsv` column is a **generated column** — PostgreSQL computes the tokenized, stemmed Spanish vector automatically on insert, and the GIN index keeps search fast even at millions of rows.
...
Which lets the agent answer things like:

- *"What did we do with Distributor A last week?"* → the relevant onboarding session.
- *"Remind me which product we said was generic on Tuesday"* → the turn where I confirmed it.
- *"The medication the admin asked about yesterday"* → found by keyword + recency.

The agent calls `recall()` as a tool. The retrieved turns get pulled into context for the current LLM call.

### Why lexical, not embeddings

I considered pgvector embeddings. Went with `tsvector` because:
...
For a "find conceptually related conversations" use case (support ticket triage, for instance), embeddings would win. For "find the specific words the admin said about X", lexical wins.

Written up as [ADR-003: PostgreSQL tsvector RAG](decisions/003-postgres-tsvector-rag.md).

## Alias learning

### The problem it solves

Suppliers name products their own way. The same `DOLXEN 500MG c/10` shows up as `DOLXEN 500MG C/10 TABLETAS`, `dolxen 500 mg tabs 10`, or `DLX-500/10`, depending on which supplier you ask. The matcher handles most of this, but **some variations are essentially arbitrary** — you can't infer the mapping from the strings alone.
...
The `(tenant_id, supplier_id, alias_normalized)` uniqueness constraint is the key point: **aliases are scoped to supplier**. `DLX-500/10` from Distributor A might map to one product; the same string from Distributor B might mean something else (or nothing).
...
When the matcher returns a high-confidence match (score ≥ 0.95) and no human intervenes, the alias is recorded with `origin = 'auto_match'`. These can be filtered out later if we stop trusting them retroactively.
...
The `times_used >= 2` threshold matters. A single admin confirmation could be a mistake; a second use means the admin (or another admin) confirmed the same mapping, so we trust it.
...
| Latency to take effect | Hours (retrain, redeploy) | Milliseconds |
...
| Reversibility | Retrain from scratch | `DELETE` or `UPDATE` |
...
- **Hit rate on the alias table grows over time** as the admin uses the system. After two months of moderate use at the pilot client, ~35% of invoice lines resolve on the alias table before the literal scorer or LLM judge ever runs.
- **Conversation memory means the admin doesn't repeat context.** *"Same as last time"* is a valid sentence — the agent looks up the previous session and works from what it finds.
- **No retraining was needed for either.** Both systems are relational tables with the right indices.
...
- **`tsvector` retrieval doesn't handle "semantically similar" queries.** *"The medicine for headaches"* won't pull up a turn about *"paracetamol"* unless the admin actually said "headache". Fine for the current use cases, but it would matter for a more general assistant.
- **`auto_match` aliases can be wrong if a high-confidence false positive slips through.** The origin column is there so I can audit and purge if needed.
