...
This isn't the "I use AI to write code" story. Everyone does that now. This is about **the engineering around the AI** — the dev environment, how I orchestrate multiple models against each other, the security sandboxes, and the custom MCP connectors that let me run real commands on real infrastructure from a chat window.
...
| Tool | What it does |
|---|---|
| `run_claude_code(prompt, working_dir, allowed_tools, session_id)` | Spawns a Claude Code session on the VPS with the given prompt. Sessions can be resumed. |
...
The server lives at `/opt/claude-code-bridge/server.py`. It's about 400 lines of Python (FastMCP plus the tunnel orchestration).
...
This was the proof that the pattern transfers. Same infrastructure, only the toolset changes per domain.

## The sandbox: how Claude Code doesn't break things

When Claude Code can run arbitrary commands on a production VPS, you need rules. One bad prompt will destroy your project otherwise.

`~/.claude/settings.json` on the VPS defines what's allowed and what isn't:
...
The rules follow one philosophy: **let it do anything reversible, block anything destructive**.

- `Write(/opt/fotech/showcase/**)` allowed because this repo is what I'm currently building.
- `Write(/opt/fotech/backend/**)` denied because the production backend should only be modified through a proper git workflow, not by chat commands.
- `Bash(rm -rf:*)` denied period. If I actually need to delete something, I do it myself.
...
The MCP bridge is synchronous — Claude calls a tool and waits. Fine for quick commands, bad for long tasks like *"sweep the matcher v3 codebase for any leftover Anthropic-direct imports and propose a refactor"*.
...
Claude Code reads `inbox/TASK-001.md`, plans, executes, writes a `REPORT-001.md` to `outbox/`. I read the report, accept it or push back, and the cycle continues.
...
For changes that matter (sprint commits, schema migrations, anything touching production), I route through Gemini CLI with a "Director Técnico de Calidad" prompt.
...
Using a **different model family** (Gemini, not Claude) is deliberate: two instances of the same model tend to share blind spots. A different model catches things Claude missed — and vice versa.

Not theoretical: Gemini has caught real issues that Claude (and I) missed. The most memorable was a migration script that would have dropped a non-empty column. Gemini's post_check flagged it as BLOCK with the actual SQL row count from the dev database.

## What this actually lets me do

Concrete things this setup unlocks:
...
2. **Debug a production issue from my phone.** I open the chat app, ask *"check why starlight is restarting"*, Claude runs `mcp__claude_code_contabo__execute_command` with `docker logs starlight --tail 50`, I see the cause, I dictate the fix, Claude Code applies it. 4 minutes on the train, done.
...
4. **Cross-session continuity without remembering anything.** Every session ends with a `SESSION-HANDOFF-CURRENT.md`. Next session, Claude reads it and we pick up exactly where we left off, including the exact technical debt I was tracking. (See [`docs/decisions/004-cache-strategy.md`](decisions/004-cache-strategy.md) for the pattern.)

## Numbers

- **Cycle time reduction**: I'd estimate 5–10× compared to my pre-augmented baseline. Tasks that used to take a day take an hour or two. Hard to measure rigorously, but it shows up in commit frequency and feature throughput.
...
- **Cost**: around $30/month in LLM calls during heavy development sessions. Trivial compared to the time saved.
...
- **It's not magic.** When the LLMs don't understand the domain, they generate plausible-looking garbage. The architecture and the pre/post checks are what catch those moments.

## Reusability

The MCP bridge pattern is reusable for any "I want to control X from a chat" problem. The Plex client proves the transfer works. Targets I'm considering for myself:

- Home Assistant (smart home from a chat)
- A personal Linux dev box from anywhere
- Kubernetes administration with sandbox rules

400 lines of `server.py` plus a fresh allow/deny ruleset turns any service into a chat-controllable one.
